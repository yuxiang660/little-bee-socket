...
#define ELPP_ASSERT(expr, msg)                                                                                         \
   ...
   {                                                                                                                   \
      ...
      ELPP_INTERNAL_DEBUGGING_OUT_ERROR << "EASYLOGGING++ ASSERTION FAILED (LINE: " << __LINE__ << ") [" #expr         \
                                        << "] WITH MESSAGE \""                                                         \
...
#define ELPP_ASSERT(expr, msg)                                                                                         \
   ...
   {                                                                                                                   \
      std::stringstream internalInfoStream;                                                                            \
      internalInfoStream << msg;                                                                                       \
      ELPP_INTERNAL_DEBUGGING_OUT_ERROR << "ASSERTION FAILURE FROM EASYLOGGING++ (LINE: " << __LINE__ << ") [" #expr   \
                                        << "] WITH MESSAGE \""                                                         \
                                        << ELPP_INTERNAL_DEBUGGING_MSG(internalInfoStream.str()) << "\""               \
                                        << ELPP_INTERNAL_DEBUGGING_ENDL;                                               \
   }
...
public:
   ...
   /// This function is useful when you want to minimize chances of failures, e.g, if you have a configuration file that
   /// sets configuration for all the configurations except for Enabled or not, we use this so that ENABLED is set to
   /// default i.e, true. If you dont do this explicitly (either by calling this function or by using second param in
   /// Constructor and try to access a value, an error is thrown
...
template <typename T> void Logger::log_(Level level, int vlevel, const T& _log)
{
   ...
   {
      base::Writer(level, "FILE", 0, "FUNCTION").construct(this, false) << _log;
   }
}
template <typename T, typename... Args>
...
template <typename T> inline void Logger::log(Level level, const T& _log)
{
   acquireLock(); // released in Writer!
   log_(level, 0, _log);
}
...
template <typename T> inline void Logger::verbose(int vlevel, const T& _log)
{
   acquireLock(); // released in Writer!
   log_(el::Level::Verbose, vlevel, _log);
}
#else
...
#define LOGGER_LEVEL_WRITERS_DISABLED(FUNCTION_NAME, LOG_LEVEL)                                                        \
   template <typename T, typename... Args> inline void Logger::FUNCTION_NAME(const char*, const T&, const Args&...)    \
   {                                                                                                                   \
      return;                                                                                                          \
   }                                                                                                                   \
...
#endif // defined(ELPP_FEATURE_ALL) || defined(ELPP_FEATURE_PERFORMANCE_TRACKING)
   /// @brief Converts template to std::string - useful for loggable classes to log containers within log(std::ostream&)
   /// const
