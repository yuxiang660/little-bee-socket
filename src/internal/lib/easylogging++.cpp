...
base::type::string_t DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit)
{
   ...
   {
      if (double(time) <= base::consts::kTimeFormats[i].value)
      {
         break;
      }
...
void TypedConfigurations::insertFile(Level level, const std::string& fullFilename)
{
   ...
   auto create = [&](Level l) {
      ...
      if (filestreamIter == m_logStreamsReference->end())
      {
         ...
         m_filenameMap.insert(std::make_pair(l, resolvedFilename));
         m_fileStreamMap.insert(std::make_pair(l, base::FileStreamPtr(fs)));
         m_logStreamsReference->insert(std::make_pair(resolvedFilename, base::FileStreamPtr(m_fileStreamMap.at(l))));
      }
      else
      {
         // Woops! we have an existing one, share it!
         m_filenameMap.insert(std::make_pair(l, filestreamIter->first));
         m_fileStreamMap.insert(std::make_pair(l, base::FileStreamPtr(filestreamIter->second)));
         fs = filestreamIter->second.get();
      }
      if (fs == nullptr)
      {
         // We display bad file error from newFileStream()
         ELPP_INTERNAL_ERROR("Setting [TO_FILE] of [" << LevelHelper::convertToString(l) << "] to FALSE", false);
         setValue(l, false, &m_toFileMap);
      }
   };
